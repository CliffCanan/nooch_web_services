...
namespace Nooch.DataAccess
{
    public class MembersDataAccess
    {
                    ...
                    using (var noochConnection = new NOOCHEntities())
                    {
                        ...
                        if (authToken != null)
                        {
                            try
                            {
                                ...
                                var firstName = CommonHelper.UppercaseFirst(CommonHelper.GetDecryptedData(CommonHelper.GetMemberDetails(MemberId).FirstName));
                                ...
                                Utility.SendEmail(Constants.TEMPLATE_REGISTRATION, fromAddress, Username, null,
                                    "Confirm your email on Nooch", link, tokens, null, null, null);
                                return "Success";
                            }
                            catch (Exception)
                            {
        ...
        public string UpdateUserLocation(string memberId, string Lat, string Long)
        {
            ...
            var memberEntity = _dbContext.Members.FirstOrDefault(m => m.MemberId == id);
            if (memberEntity != null)
            {
                ...
                _dbContext.Entry(memberEntity).ReloadAsync();
                return "success";
            }
                    ...
                    default:
                            ...
                            memberEntity.Type == "Personal - Browser")
                        {
                            ...
                            else if (loginRetryCountInDb == 3)
                            {
                                ...
                                Logger.Info("MDA -> LoginRequest FAILED - User's PW was incorrect - 4th Invalid Attempt, now suspending user - UserName: [" + userName + "]");
        ...
        public synapseCreateUserV3Result_int RegisterUserWithSynapseV3(string memberId)
        {
            ...
            if (noochMember != null)
            {
                ...
                if (synapseCreateUserObjIfExists != null)
                {
                    try
                    {
                        Logger.Info("MDA -> RegisterUserWithSynapseV3 - User ALREADY has a Synapse account - Calling refreshSynapseV3OautKey()");

                        var refreshTokenResult = CommonHelper.refreshSynapseV3OauthKey(synapseCreateUserObjIfExists.access_token);
                                                                               ...
                                                                               string isIdImageAdded = "0", string idImageData = "")
        {
            ...
            if (!String.IsNullOrEmpty(memberIdFromPhone))
            {
                ...
                else
                {
                    ...
                    //              on the page (which could be CreateAccount, PayRequest or DepositMoney)
                    var error = "MDA -> RegisterNonNoochUserWithSynapseV3 FAILED - PHONE Already Registered: [" + userPhone + "], Email: [" + userEmail + " - ABORTING";
                    CommonHelper.notifyCliffAboutError(error);
                    Logger.Error(error);
        ...
        {
            Logger.Info("MDA - CreateNonNoochUserAccountAfterRejectMoney Fired - TransID: [" + TransId + "], " +
            ...
            try
            {
                ...
                if (transactionDetail != null)
                {
                    ...
                    if (memdetails == null)
                    {
                        ...
                        var emailEnc = CommonHelper.GetEncryptedData(userNameLowerCase);
                        ...
                        if (inviteCode.Length > 0) member.InviteCodeIdUsed = Utility.ConvertToGuid(inviteCode);
        ...
        /// <summary>
        /// For verifying a Synapse bank account attached w/ Routing & Account #'s. The user must confirm 2 micro-deposit payments made to their bank account.
        /// </summary>
        /// <param name="MemberId"></param>
        ...
        {
            Logger.Info("MDA -> SynapseV3MFABankVerifyWithMicroDeposits Fired - MemberID: [" + MemberId +
                        "], Micro1: [" + microDepositOne + "], Micro2: [" + microDepositTwo + "], BankID (enc): [" + BankId + "]");
                ...
                String.IsNullOrEmpty(BankId))
            {
                ...
                Logger.Error("MDA -> SynapseV3MFABankVerifyWithMicroDeposits FAILED - Missing Info - ABORTING: [" + res.errorMsg + "], MemberID: [" + MemberId + "]");
                return res;
            }
            ...
            if (noochMember != null)
            {
                ...
                if (noochMemberResultFromSynapse != null)
                {
                    ...
                    if (bankAccountDetails != null)
                    {
                        ...
                        try
                        {
                                ...
                                bankLoginRespFromSynapse["nodes"] != null)
                            {
                                ...
                                // CC (9/120/16): Commenting out this line - not sure why it's needed for this service. Users
                                // that add a bank w/ routing/account #'s probably wouldn't have a Bank Login result anyway.
                                ...
                                if (allNodesParsedResult != null)
                                {
                                    ...
                                    {
                                        var error = "MDA -> SynapseV3MFABankVerifyWithMicroDeposits ERROR: Failed to update this bank in DB - " +
                                ...
                                {
                                    var error = "MDA -> SynapseV3MFABankVerifyWithMicroDeposits ERROR: Unable to parse Bank Nodes Array in Synapse Response.";
                                    Logger.Error(error);
                                    CommonHelper.notifyCliffAboutError(error);
                            ...
                            else
                            {
                                ...
                                var error = "MDA -> SynapseV3MFABankVerifyWithMicroDeposits ERROR: Synapse response for 'success' was not true for Member [" + MemberId + "]";
                                Logger.Error(error);
                                CommonHelper.notifyCliffAboutError(error);
                                res.errorMsg = "Synapse response for success was not true";
                            }

                            Logger.Info("MDA -> SynapseV3MFABankVerifyWithMicroDeposits - Result - Success: [" + res.Is_success.ToString() + "], Msg: [" + res.errorMsg + "]");
                        ...
                        catch (WebException we)
                        {
                            ...
                            var error = "MDA -> SynapseV3MFABankVerifyWithMicroDeposits FAILED - Response was: [" + jsonfromsynapse + "], Exception: [" + we.Message + "]";
                            Logger.Error(error);
                            CommonHelper.notifyCliffAboutError(error);

                            JToken token = jsonfromsynapse["error"]["en"];

                            if (token != null) res.errorMsg = jsonfromsynapse["error"]["en"].ToString();
                        }
                    ...
                    {
                        var error = "MDA -> SynapseV3MFABankVerifyWithMicroDeposits ERROR: Member's Bank not found - MemberID: [" + MemberId + "], And BankID: [" + BankId + "]";
                        Logger.Error(error);
                        CommonHelper.notifyCliffAboutError(error);
        ...
        public string MemberRegistration(byte[] Picture, string UserName, string FirstName, string LastName, string PinNumber,
            string Password, string SecondaryMail, string UUID, string friendRequestId, string invitedFriendFacebookId,
            string facebookAccountLogin, string inviteCode, string sendEmail, string type, string phone)
        {
                ...
                if (!String.IsNullOrEmpty(noochRandomId))
                {
                    ...
                    if (!String.IsNullOrEmpty(inviteCode))
                    {
                        try
                        {
                            ...
                            if (inviteCodeObj != null)
                            {
                                if (inviteCodeObj.count >= inviteCodeObj.totalAllowed) //Removed (inviteCodeObj == null || inviteCodeObj.count >= inviteCodeObj.totalAllowed) this condition, b/c user can reg without Invite code -Surya 6-May-16
                                {
                    ...
                    var member = new Member
                    {
                        ...
                        Photo = Utility.GetValueFromConfig("PhotoUrl") + "gv_no_photo.png", //Setting default here, will update below if Picture data were passed
                    ...
                    if (result > 0)
                    {
                        ...
                        if (status > 0)
                        {
                            ...
                            if (!String.IsNullOrEmpty(inviteCode) && inviteCode.ToLower() != "nocode")
                            {
                                try
                                {
                                    // Sending email to user who invited this user (Based on the invite code provided during registration)
                                    var fullName = CommonHelper.UppercaseFirst(FirstName) + " " + CommonHelper.UppercaseFirst(LastName);
                            ...
                            try
                            {
                                var fbTxt = !String.IsNullOrEmpty(facebookAccountLogin) && facebookAccountLogin.Length > 5
                                ...
                                StringBuilder st = new StringBuilder("<p><strong>This user's Nooch Account information is:</strong></p>" +
                                                      ...
                                                      "<tr><td><strong>Facebook:</strong></td><td>" + fbTxt + "</td></tr>" +
                                                      "<tr><td><strong>Invite Code:</strong></td><td>" + inviteCode +
                                                      "</td></tr></table><br/><br/>- Nooch Bot</body></html>");

                                // Notify Nooch Admin
                                StringBuilder completeEmailTxt = new StringBuilder();
                                var s = "<html><body><h2>New Nooch User Created</h2><p>The following person just created a Nooch account <strong>from the mobile app</strong>:</p>" + st.ToString() +
                                        "<br/><br/><small><strong>This email was generated automatically in: [MDA -> MemberRegistration]</strong></small></body></html>";

                                completeEmailTxt.Append(s);

                                Utility.SendEmail(null, "admin-autonotify@nooch.com", "newUser@nooch.com", null,
                                                  "Nooch Alert - NEW USER: " + FirstName + " " + LastName,
                                                  null, null, null, null, completeEmailTxt.ToString());
                            }
                            catch (Exception ex)
                            {
                                            ...
                                            foreach (FBMemberDataClass FbUser in FbFriends)
                                            {
                                                if (!String.IsNullOrEmpty(FbUser.id))
                                                {
                                                    ...
                                                    if (noochMember != null)
                                                    {
                                                        ...
                                                        {
                                                            var existingUserFirstName = CommonHelper.UppercaseFirst(CommonHelper.GetDecryptedData(noochMember.FirstName));
                                                            var existingUserLastName = CommonHelper.UppercaseFirst(CommonHelper.GetDecryptedData(noochMember.LastName));
                                                            var newUserFirstName = CommonHelper.UppercaseFirst(CommonHelper.GetDecryptedData(member.FirstName));
                                                            var newUserLastName = CommonHelper.UppercaseFirst(CommonHelper.GetDecryptedData(member.LastName));

                                                            var newUserPictureUrl = "";
        ...
        {
            Logger.Info("MDA -> LogoutRequest Fired - MemberID: [" + memberId + "]");
